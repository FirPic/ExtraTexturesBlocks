package fr.firpic.extratexturesblocks;


import fr.firpic.extratexturesblocks.items.EtbItemsRegistry;
import net.minecraft.item.ItemGroup;
import net.minecraft.item.ItemStack;

public class EtbItemGroup {

    public static final ItemGroup EtbGroup = new ItemGroup("Extra Textures Blocks") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    };

    public static final ItemGroup AncientStoneGroup = new ItemGroup("Ancient Stone") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    };
    public static final ItemGroup AndesiteGroup = new ItemGroup("Andesite") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    };
    public static final ItemGroup AracaneStoneGroup = new ItemGroup("Arcane Stone") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    };
    public static final ItemGroup BricksGroup = new ItemGroup("Bricks") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    };
    public static final ItemGroup CertusGroup = new ItemGroup("Certus") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    };
    public static final ItemGroup CharcoalGroup = new ItemGroup("CharCoal") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    };
    public static final ItemGroup CoalGroup = new ItemGroup("Coal") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    };
    public static final ItemGroup CoalcokeGroup = new ItemGroup("CoalCoke") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    };
    public static final ItemGroup CobblestoneGroup = new ItemGroup("Cobblestone") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    };
    public static final ItemGroup DiabaseGroup = new ItemGroup("Diabase") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    };
    public static final ItemGroup DioriteGroup = new ItemGroup("Diorite") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    };
    public static final ItemGroup EndstoneGroup = new ItemGroup("End Stone") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    }; 
    public static final ItemGroup FluidGroup = new ItemGroup("Fluid ") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    }; 

    public static final ItemGroup GlowstoneGroup = new ItemGroup("Glowstone ") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    }; 
    public static final ItemGroup GraniteGroup = new ItemGroup("Granite ") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    }; 
    public static final ItemGroup IceGroup = new ItemGroup("Ice ") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    }; 
    public static final ItemGroup LimestoneGroup = new ItemGroup("Limestone ") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    }; 
    public static final ItemGroup MagmaGroup = new ItemGroup("Magma ") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    }; 
    public static final ItemGroup MarbleGroup = new ItemGroup("Marble ") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    }; 
    public static final ItemGroup PrismarineGroup = new ItemGroup("Prismarine ") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    }; 
    public static final ItemGroup PurpurGroup = new ItemGroup("Purpur ") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    }; 
    public static final ItemGroup QuartzGroup = new ItemGroup("Quartz ") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    }; 
    public static final ItemGroup RedSandstoneGroup = new ItemGroup("Red Sandstone ") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    };
    
    public static final ItemGroup RedstoneGroup = new ItemGroup("Redstone ") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    }; 
    public static final ItemGroup SandstoneGroup = new ItemGroup("Sandstone ") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    };
    public static final ItemGroup StonebricksGroup = new ItemGroup("Stone Bricks ") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    }; 
    public static final ItemGroup TerracottaGroup = new ItemGroup("Terracotta ") {
        @Override
        public ItemStack createIcon() {
            return new ItemStack(EtbItemsRegistry.TRANF.get());
        }
    }; 
}

